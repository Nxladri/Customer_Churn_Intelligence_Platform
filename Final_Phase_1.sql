/* The final feature table for the customer churn prediction model
1.CustomerID(Unique) , 2.Total Revenue(Monetary) , 3. Order Frequency(Frequency) , 4.Average OrderValue(Temporal) , 
5. Days since last order(Recency) , 6. Avg Days Between Orders(Behavioural) , 7.Customer Tenure (Loyality),
8. Last Order Year (which year they stopped ordering) , 9. TotalProductCategories (Variety of Purchase),
10. Churn Label (Target => 0/1) , (additional)11. MostPurchasedCategories*/
...
--Main Query
SELECT * FROM LastButNotTheLeast
ORDER BY CustomerID
